...
impl<C: OpaqueCiphersuite> ClientLoginState<C> {
    ...
    pub fn finish(
        ...
        let mut transcript2_input = Zeroizing::new(Vec::with_capacity(
            preamble.len() + expected_server_mac.len(),
        ));
...
impl<C: OpaqueCiphersuite> ServerLogin<C> {
    ...
    pub fn start_fake(
        ...
        let mut transcript2_input =
            Zeroizing::new(Vec::with_capacity(preamble.len() + server_mac.len()));
        transcript2_input.extend_from_slice(&preamble);
