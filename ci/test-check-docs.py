...
def main() -> int:
    ...
    failures = []
    try:
        for name, why, build, run, want_finding in CASES:
    ...
    print()
    if failures:
        print(f"{len(failures)} case(s) not handled as expected: {failures}")
        print(
            "Against the current checker this is a real regression; against "
            "an older copy passed with --script it is the expected result, "
            "and is what makes the fix meaningful."
        )
        return 1
