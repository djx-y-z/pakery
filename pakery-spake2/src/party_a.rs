...
impl<C: Spake2Ciphersuite> PartyA<C> {
    ...
    fn start_inner(
        ...
        // ctgrind: pA is the wire key share — public by protocol design.
        pakery_core::ct::declassify(&pa_bytes);
...
mod tests {
    ...
    fn zeroize_clears_all_secret_fields() {
        let mut state = PartyAState::<MockSuite> {
            x: [0xAA; 32],
            w: [0xBB; 32],
            pa_bytes: vec![0xCC; 32],
        ...
        assert!(state.pa_bytes.is_empty());
